...
func SaveMetric(w *http.ResponseWriter, metric Metric, metricName string) {
	...
	err := os.WriteFile(filepath.Join(os.Getenv(BaseDirEnv), "map.txt"), []byte(MapMetric.String()), 0644)
	if err != nil {
		panic(err)
	}
	(*w).WriteHeader(http.StatusOK)
	status, err := (*w).Write([]byte(fmt.Sprintf("updated mapMetricGauge: %v", MapMetric)))
	if err != nil {
